...
############ MAAL: added burnin to remove some burnin indicared by user
Runtwalk <- function(Tr, Obj, Supp, dat, dim = length(x0), x0=x0, xp0=xp0, at=6, aw=1.5, pphi=min( dim, 4)/dim, F1=0.4918, F2=F1+0.4918, F3=F2+0.0082, thinning=100,burnin=1e+3, cumulative=FALSE, out.fl=c(), energy.fl=c(), ...) {
  ...
  j <- 0
  i_burn <- 0
  pb_burn <- txtProgressBar(min=0, max=burnin, style = 3, char="=")
  ...
  while(j < Tr) {
    ...
    if( (i_burn %% thinning) == 0 && i_burn >= burnin ) { # then store the iteration
...
Ana <- function(info, from=1, to=info$Tr, par=0, file="",mensaje=TRUE) {
  sel <- from:to
  ...
  #### No plots
  if (mensaje){  Tint <- IAT( info, par=par,to=to)  ### defined below
  ...
  return(c(info$acc/info$Tr,IAT( info, par=par,to=to),IAT( info, par=par,to=to)/info$dim))
  }
